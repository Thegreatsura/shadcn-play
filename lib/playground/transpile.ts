...
const IMPORT_REGEX = /(?:import|export)\s+(?:[\s\S]*?from\s+)?['"]([^'"]+)['"]/g
...
function findImportLine(code: string, specifier: string): { line: number; column: number } {
...
export function transpileTSX(source: string): TranspileResult {
  ...
  try {
    ...
    return { js, imports: sourceImports }
  } catch (err: unknown) {
